...
def dex2X(tmpDir, dexFile):
    ...
    baksmali = subprocess.Popen(
        ['java', '-Xmx256M', '-jar', settings.BACKSMALI, '-o', smaliLocation,
         dexFile])
    baksmali.wait()
...
def getPermissions(logFile, sampleFile):
    ...
    permissions = permissions.communicate(0)[0].split("uses-permission: ".encode())
